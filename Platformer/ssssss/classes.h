...
public:
	...
	void update(float time)
	{
		...
		{
			timer += time;
			if (timer > 1200) { superAttack = false; timer = 0; }
			else { anim.set("light_shoot"); dx = 0; }
		}
...
public:
	...
	void dead(float time)
	{
		if (Health <= 0)
		{
			...
			if (timer_anim_dead > 1500) { life = false; timer_anim_dead = 0; }
			else anim.set("dead");
		}
	}
...
//		s.setPosition(center.x - size.x / 2 + 120, center.y - size.y / 2 + 10);
	}
	...
	void draw(RenderWindow &window)
	{
		...
		s.setPosition(center.x - size.x / 2 + 120 - removal / 6.7, center.y - size.y / 2 + 10);
